# Pharma Supply Manager

Sistema de gestión de cadena de suministro farmacéutica desarrollado con Node.js, Express y SQL Server.

## Descripción
Pharma Supply Manager permite gestionar productos, proveedores, sucursales y stock dentro de una red farmacéutica.  
El objetivo es optimizar la trazabilidad de los medicamentos y mejorar la eficiencia operativa.

## Tecnologías
- Node.js  
- Express  
- SQL Server  
- CORS  
- Body-parser

##  Instalación y ejecución

1. Clonar el repositorio:
   ```bash
   git clone https://github.com/CarolinaTesando/pharma-supply-manager.git
   ```

2. Ir a la carpeta del backend:
   ```bash
   cd Backend
   ```

3. Instalar dependencias:
   ```bash
   npm install
   ```

4. Ejecutar el servidor:
   ```bash
   npm run dev
   ```

## Autora
**Carolina Tesando**  
Estudiante de Tecnicatura Superior en Desarrollo de Software | IFTS 24|
